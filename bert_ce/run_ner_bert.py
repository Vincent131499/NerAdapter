...
flags.DEFINE_string(
    "data_dir", '../china-people-daily-data-lie',
    "The input datadir.",
)

flags.DEFINE_string(
    "bert_config_file", '../pretrained_models/chinese_L-12_H-768_A-12/bert_config.json',
...
flags.DEFINE_string("export_dir_base", './exported_model',
                    "The output directory where the pb model will be written")

## Other parameters
flags.DEFINE_string(
    "init_checkpoint", '../pretrained_models/chinese_L-12_H-768_A-12/bert_model.ckpt',
...
flags.DEFINE_bool("do_infer", True, "Whether to run the model in inference mode on the test set.")
...
flags.DEFINE_string("vocab_file", '../pretrained_models/chinese_L-12_H-768_A-12/vocab.txt',
...
def filed_based_convert_examples_to_features(
    ...
    for (i, label) in enumerate(label_list, 1):
        label_map[label] = i
    with open('./{}/label2id.pkl'.format(FLAGS.output_dir), 'wb') as w:
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        with open('./{}/label2id.pkl'.format(FLAGS.output_dir), 'rb') as rf:
