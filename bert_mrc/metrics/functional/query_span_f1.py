...
def query_span_f1(start_preds, end_preds, match_logits, start_label_mask, end_label_mask, match_labels, flat=False):
    ...
    return torch.stack([tp, fp, fn]), match_preds


def query_span_f1_all_tag(start_preds, end_preds, match_logits, start_label_mask, end_label_mask, match_labels,
                          flat=False):
    """
    Compute span f1 according to query-based model output
    Args:
    ...
    tp = (match_labels & match_preds).long().sum(dim=2).sum(dim=1)
    fp = (~match_labels & match_preds).long().sum(dim=2).sum(dim=1)
    fn = (match_labels & ~match_preds).long().sum(dim=2).sum(dim=1)
    return tp, fp, fn, match_preds  # batch
