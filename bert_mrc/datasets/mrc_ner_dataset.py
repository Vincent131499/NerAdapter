...
class MRCNERDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        new_start_positions = [start + len(query) + 2 for start in start_positions]
        new_end_positions = [end + len(query) + 2 - 1 for end in end_positions]
...
class MRCNERDataset_test(Dataset):
    """
    MRC NER Dataset
    Args:
        json_path: path to mrc-ner style json
...
def run_dataset():
    ...
    # zh datasets
    bert_path = "../prev_trained_model/bert-base"
    json_path = "../data/ccfner_mrc_format/mrc-ner.train"
    ...
    for index, batch in enumerate(dataloader):
        ...
        print('match_labels_bool', match_labels.bool().sum())
